...
describe('Child with both parents and grandparents', () => {
  ...
  beforeEach(async () => {
    ...
    const savedMotherNode = await createPersonNode(savedMotherPerson, [], []);
    motherNodeId = savedMotherNode;

    // Create and save father person node
    const savedFatherPerson = await createPerson('Father Person');
    const savedFatherNode = await createPersonNode(savedFatherPerson, [], []);
    ...
    const savedMaternalGrandmotherNode = await createPersonNode(savedMaternalGrandmother, [], []);
    maternalGrandmotherId = savedMaternalGrandmotherNode;

    const savedMaternalGrandfather = await createPerson('Maternal Grandfather');
    const savedMaternalGrandfatherNode = await createPersonNode(savedMaternalGrandfather, [], []);
    ...
    const savedPaternalGrandmotherNode = await createPersonNode(savedPaternalGrandmother, [], []);
    paternalGrandmotherId = savedPaternalGrandmotherNode;

    const savedPaternalGrandfather = await createPerson('Paternal Grandfather');
    const savedPaternalGrandfatherNode = await createPersonNode(savedPaternalGrandfather, [], []);
  ...
  test('checking ancestor relationships', async () => {
    ...
    let res = await api
      ...
      .send({ referenceId: motherNodeId.toString(), destinationId: childNodeId.toString() })
      .expect(200)
    ...
    res = await api
      .post('/api/trees/check-relationship')
      .send({ referenceId: fatherNodeId.toString(), destinationId: childNodeId.toString() })
      .expect(200)
      .expect('Content-Type', /application\/json/);
    ...
    res = await api
      .post('/api/trees/check-relationship')
      .send({ referenceId: maternalGrandmotherId.toString(), destinationId: childNodeId.toString() })
      .expect(200)
      .expect('Content-Type', /application\/json/);
    ...
    res = await api
      .post('/api/trees/check-relationship')
      .send({ referenceId: maternalGrandfatherId.toString(), destinationId: childNodeId.toString() })
      .expect(200)
      .expect('Content-Type', /application\/json/);
    ...
    res = await api
      .post('/api/trees/check-relationship')
      .send({ referenceId: paternalGrandmotherId.toString(), destinationId: childNodeId.toString() })
      .expect(200)
      .expect('Content-Type', /application\/json/);
    ...
    res = await api
      .post('/api/trees/check-relationship')
      .send({ referenceId: paternalGrandfatherId.toString(), destinationId: childNodeId.toString() })
      .expect(200)
      .expect('Content-Type', /application\/json/);
